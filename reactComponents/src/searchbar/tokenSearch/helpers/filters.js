...
export const filterValidExchangeNames = networkNames => Array.from(new Set(networkExchangePairs.filter(network => filterActiveNames(networkNames).includes(network[0])).map(network => network[1])));